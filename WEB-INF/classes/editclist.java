...
public class editclist extends HttpServlet {
	...
	r=ob.getResultSet(sq);
	    ...
	    while(r.next()){
		      out.println("<option >"+r.getString("catagory")+"</option>");
	...
	if(submit.equals("Modify"))
	{
		...
		String sql3="SELECT * from dvdcat where catagory like '%"+dvds+"'";
	...
	ResultSet rc=ob.getResultSet(sql3);
			 ...
			 out.println("<input type='text'  value='Catagory' disabled/><br />");
	    ...
	    while(rc.next()){
			 ...
			 out.println("<input type='text' name='catagory' value='"+rc.getString("catagory")+"'/>");
	...
	ob.executeQuery(stm);
	 ...
	 String stm="update  dvdcat set catagory='"+catagory+"' where id="+id+"";
	 //out.println(stm);
	  try{
	ob.executeQuery(stm);
		 //out.println(stm);
